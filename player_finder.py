...
def collect_player_data():
    ...
    player["color"] = input(
        "Which color you want to prepare against (white/black)?  "
    )
    return player
...
def player_finder(player):
    ...
    # step 1: parse main CR page,
    # search for player, return file with tournaments played
    parse_main_page(player["id"], tournaments_filepath)

    # step 2: from the list of tournaments played,
    # search those with downloadable games
...
def main():
    ...
    if len(sys.argv) != 4:
        ...
        player = {"id": sys.argv[1], "name": sys.argv[2], "color": sys.argv[3]}
