...
def extract_players_from_table(table_rows):
    ...
    for row in table_rows:
        ...
        if player["rtg"] is None or player["rtg"] == "0":
            print(
                "SKIPPING PLAYER: %s does not have FIDE ID and/or rating"
                % player["name"]
            )
        else:
            print(
                "ADDING PLAYER: %s %s %s"
                % (player["id"], player["name"], player["rtg"])
            )
...
def search_player_in_form(driver, keyword):
    ...
    # (you can adjust this part to extract specific information)
...
def scrap_page(driver, url, player_id):
    """
    Navigate to the chess-results website,
    search for player and collect results
    """
    driver.get(url)
...
def get_tournament_details(link, tournament_driver):
    ...
    # tournaments that finished more than 5 days ago
    # will require extra processing
    if "cb_alleDetails" in tournament_source:
        alle_details_button = tournament_driver.find_element(
            "id", "cb_alleDetails"
        )
    ...
    if "games available" in tournament_source:
        games_link = tournament_driver.find_element(
            "xpath", "//a[contains(@href, 'PartieSuche.aspx?lan=1&id=')]"
        ).get_attribute("href")
...
def download_pgn(games_link, expected_file_name):
    ...
    wait.until(
        lambda games_driver: expected_file_name in os.listdir(download_dir)
    )
...
def download_all_pgns(tournaments_filepath):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        futures = [
            executor.submit(download_tournament_details, link)
...
def parse_main_page(player_id, tournaments_filepath):
    ...
    tournaments_played_source = scrap_page(
        chess_results_driver, main_chess_results_url, player_id
    )
    ...
    list_of_links = extract_tournament_details_from_table(
        tournaments_table_rows
    )
...
def parse_tournament_page(tournament_id):
    ...
    # Find all players
    tournament_players_rows = soup.find_all(
        "tr", class_=["CRng1 BRA", "CRng2 BRA"]
    )
